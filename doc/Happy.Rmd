...
packages.used=c('tm', 'tidytext', 'tidyverse', 'DT', 'ggplot2', 'ggpubr', 'plotly', 'wordcloud')
...
```{r, echo = FALSE}
count_24_age <- table(as.integer(demo_data_24$age))
count_24_age[order(-count_24_age)][1:10]
```

3 months age frequency
```{r, echo = FALSE}
count_3_age <- table(as.integer(demo_data_3$age))
count_3_age[order(-count_3_age)][1:10]
```
...
par(mfrow=c(1,2))
plot_ly(all, labels = ~label, values = ~value_24, type = 'pie',
        textposition = 'inside',
...
plot_ly(all, labels = ~label, values = ~value_3, type = 'pie',
        textposition = 'inside',
        textinfo = 'label+percent',
        insidetextfont = list(color = '#FFFFFF'),
        hoverinfo = 'text',
